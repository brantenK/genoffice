...
// A plain settings entry: clicking it opens the settings modal directly
// (SettingsModal.tsx), which hosts the fork's preferences (language, theme,
// save location, update channel). The upstream Genspark sign-in flow is
// deliberately not wired up here — see fork/COMPLIANCE.md.
...
export function Home() {
  ...
  // calm. Each entry runs the same handler the old quick-create cards did.
  ...
  const OFFICE_APP_ITEMS: AppNavItem[] = [
    {
      ext: 'docx',
      label: t('appNavNewDoc'),
      action: () => void window.aiOffice.newDoc(newFileOpts),
    },
    {
      ext: 'xlsx',
      label: t('appNavNewSheet'),
      action: () => void window.aiOffice.newSheet(newFileOpts),
    },
    {
      ext: 'pptx',
      label: t('appNavNewSlide'),
      action: () => void window.aiOffice.newSlide(newFileOpts),
    },
    {
      ext: 'pdf',
      label: t('appNavNewPdf'),
      action: () => void window.aiOffice.newPdf(newFileOpts),
    },
    {
      ext: 'md',
      label: t('appNavNewMd'),
      action: () => void window.aiOffice.newMarkdown(newFileOpts),
    },
    {
      ext: 'html',
      label: t('appNavNewHtml'),
      action: () => void window.aiOffice.newHtml(newFileOpts),
    },
  ]
