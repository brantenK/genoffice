...
describe('root development launcher', () => {
  it('keeps the renderer topology and launches the shell with cross-platform environment variables', () => {
    ...
    const shellCommand =
      'cross-env DOCS_RENDERER_URL=http://localhost:5173 SHEETS_RENDERER_URL=http://localhost:5174 SLIDES_RENDERER_URL=http://localhost:5175 PDF_RENDERER_URL=http://localhost:5176 MARKDOWN_RENDERER_URL=http://localhost:5177 CRM_RENDERER_URL=http://localhost:5178 TENDERS_RENDERER_URL=http://localhost:5179 BOOKS_RENDERER_URL=http://localhost:5180 npm run dev -w @genoffice/shell'

    expect(dev).toBeDefined()
    expect(dev).toMatch(
      /^concurrently -k -n docs,sheets,slides,pdf,markdown,crm,tenders,books,shell -c blue,green,yellow,red,cyan,purple,orange,teal,magenta /,
    )
