...
export interface TenderRule {
  ...
  riskLevel: RiskLevel
  evidenceKind: 'DOCUMENT' | 'SIGNATURE' | 'NONE'
...
// OCR/punctuation-tolerant separator between the letters of B-BBEE / B-BEE:
// spaces, dots, hyphens (ASCII and Unicode) and slashes all occur in the wild.
const BEE_SEP = '[\\s.\\u00b7/\\-\\u2010-\\u2015\\u2212]*'
...
export const TENDER_RULES: TenderRule[] = [
  ...
  },
  {
    ...
    order: 30,
    patterns: [
      // B-BBEE / BBBEE / B - B B E E / B.BBEE (OCR + punctuation variants)
      rx(`\\bb${BEE_SEP}b${BEE_SEP}b${BEE_SEP}e${BEE_SEP}e\\b`),
      // B-BEE variant
      rx(`\\bb${BEE_SEP}b${BEE_SEP}e${BEE_SEP}e\\b`),
      rx('broad[\\s\\-\\u2010-\\u2015]*based\\s+black\\s+economic\\s+empowerment'),
      rx(
        '\\bbee\\b[^.]{0,30}(certificate|affidavit|status|level|recognition|scorecard|verified|compliant|rating)',
      ),
      rx('empowerment\\s*(status|level|certificate|rating)'),
      rx('sworn\\s*affidavit[^.]{0,30}(b?bee|empowerment)'),
    ],
    vaultHints: { category: 'COMPLIANCE', keywords: ['bbbee', 'bee', 'affidavit', 'empowerment'] },
    notes:
      'EME/QSE affidavits are valid for 12 months; generic certificates valid until the next verification.',
  },
  {
    key: 'cipc',
    ...
    vaultHints: {
      category: 'GOVERNANCE',
      keywords: ['cipc', 'registration', 'ck', 'cor', 'incorporation'],
    },
  },
  {
    key: 'director_ids',
    ...
    patterns: [
      ...
      rx(
        '(director|member|partner|owner|shareholder)s?[^.]{0,60}(id|identity|passport)\\s*(copy|copies|document)',
      ),
      rx('(id|identity)\\s*(copy|copies|documents?)[^.]{0,40}(director|member|owner)'),
      // OCR wording drift: "identity papers/documents", "directors' identity
      // documents" (identity before the person noun).
      rx(
        "(director|member|partner|owner|shareholder|trustee)s?'?\\s+(id|identity|passport)\\s*(documents?|copies?|papers?)",
      ),
      rx(
        '(id|identity|passport)\\s*(papers?|documents?|cop(?:y|ies))[^.]{0,40}(director|member|owner|partner|shareholder)',
      ),
    ],
    vaultHints: {
      category: 'COMPLIANCE',
      keywords: ['id copy', 'identity', 'passport', 'certified id', 'director'],
    },
  ...
  },
  {
    ...
    patterns: [
      ...
      // OCR wording drift: "endorsed by a Commissioner of Oaths", "dated
      // within 90 days".
      rx('endors\\w*[^.]{0,40}(commissioner of oaths|police\\b)'),
      rx(
        '(commissioner of oaths|police\\b)[^.]{0,60}(within\\s*90\\s*days|dated|not older than|attest|stamp|certif)',
      ),
    ],
    negative: [rx('good standing')],
    vaultHints: { category: 'COMPLIANCE', keywords: ['certified', 'police', 'oaths', 'id copy'] },
    notes:
      'Zanostack Tenders flags any vault document whose certification stamp is older than 90 days.',
  },
  {
    key: 'csd',
    ...
    order: 70,
    patterns: [
      rx('central supplier database'),
      rx('\\bcsd\\b'),
      rx('supplier\\s*(database|registration|number)'),
    ],
  ...
  },
  {
    ...
    patterns: [
      rx('\\b(sbd|mbd)\\s*\\d'),
      rx('returnable\\s*(document|form|s)?\\b'),
      rx('\\bannexure\\s*[a-z]\\b'),
    ],
  ...
  },
  {
    ...
    order: 110,
    patterns: [
      rx('original\\s*(documents?|copies?|certificates?|receipts?)'),
      rx('no\\s*(photocopies|copies)\\s*(will be\\s*)?accepted'),
    ],
  ...
  },
  {
    ...
    order: 120,
    patterns: [
      rx('validity\\s*(period|of)\\s*(the\\s*)?(bid|offer|proposal|quotation|tender)?'),
      rx('bids?\\s*(remain|valid)\\s*valid\\s*for'),
      rx('valid\\s*for\\s*(a\\s*(further\\s*)?)?\\d+\\s*(days?|months?)'),
    ],
  ...
  },
  {
    ...
    vaultHints: {
      category: 'TECHNICAL',
      keywords: ['experience', 'reference', 'track record', 'project'],
    },
  },
  {
    key: 'key_personnel',
    ...
    patterns: [
      ...
      rx(
        '(professional|engineer|technician|manager)s?\\s*(qualifications?|registration|certificates?)',
      ),
      rx('\\bpr\\s*(eng|tech)\\b'),
    ...
    vaultHints: {
      category: 'CV',
      keywords: ['cv', 'curriculum', 'personnel', 'qualification', 'engineer'],
    },
  },
  {
    key: 'methodology',
    ...
    patterns: [
      rx('methodolog\\w*'),
      rx('work\\s*(programme|program|plan|schedule)'),
      rx('implementation\\s*(plan|programme|approach)'),
      rx('project\\s*(plan|approach|method)'),
    ],
  ...
  },
  {
    ...
    patterns: [
      ...
      // OCR wording drift: "statements of audited accounts", "audited accounts".
      rx('statements?\\s*of\\s*(audited\\s*)?accounts?'),
      rx('audited\\s*accounts?'),
    ],
    vaultHints: {
      category: 'FINANCIAL',
      keywords: ['financial', 'audited', 'statements', 'management accounts'],
    },
  },
  {
    key: 'turnover',
    ...
    order: 310,
    patterns: [
      rx('annual\\s*turnover'),
      rx('turnover\\s*(of|r\\s*\\d|exceeding|at least|minimum)'),
      rx('average\\s*annual\\s*turnover'),
    ],
  ...
  },
  {
    ...
    order: 330,
    patterns: [
      rx('preferential procurement'),
      rx('(80|90)\\s*[:/]\\s*(20|10)'),
      rx('pppfa'),
      rx('preferential points?'),
    ],
  ...
  },
  {
    ...
    vaultHints: {
      category: 'GOVERNANCE',
      keywords: ['sub-contract', 'subcontract', 'enterprise development', 'local content'],
    },
  },
  {
    key: 'pricing_schedule',
    title: 'Completed Pricing Schedule / Price Proposal',
    category: 'FINANCIAL_STAGE_3',
    riskLevel: 'CRITICAL_DISQUALIFIER',
    // A completed pricing schedule is a document-backed returnable; the
    // FINANCIAL vault category backs it (see readiness evidence review).
    evidenceKind: 'DOCUMENT',
    validityKind: 'NONE',
    order: 350,
    patterns: [
      rx('pric(e|ing)\\s*(schedule|list|form|proposal|table|breakdown)'),
      rx('schedule\\s*of\\s*(rates|prices|quantities|amounts)'),
      rx('rates?\\s*(and|&)?\\s*prices?'),
      rx('unit\\s*(rates?|prices?)'),
      rx('priced\\s*(schedule|bill|boq|pricing)'),
      rx('completed\\s*(pricing|price|rates?)\\s*(schedule|form|list)?'),
      rx('\\bcosting\\s*schedule\\b'),
    ],
    vaultHints: {
      category: 'FINANCIAL',
      keywords: ['pricing', 'price', 'rates', 'schedule', 'costing'],
    },
    notes: 'A missing or unpriced schedule is normally non-responsive.',
  },
  {
    key: 'bill_of_quantities',
    title: 'Bill of Quantities (BOQ) / Bill of Materials',
    category: 'FINANCIAL_STAGE_3',
    riskLevel: 'POINT_SCORED',
    // A priced BOQ/BoM is a document-backed returnable; the FINANCIAL vault
    // category backs it (see readiness evidence review).
    evidenceKind: 'DOCUMENT',
    validityKind: 'NONE',
    order: 360,
    patterns: [
      rx('bill\\s*of\\s*(quantities|quantity|materials|works)'),
      rx('\\bboq\\b'),
      rx('\\bbom\\b'),
    ],
    vaultHints: {
      category: 'FINANCIAL',
      keywords: ['bill of quantities', 'boq', 'bill of materials', 'bom'],
    },
    notes: 'BOQ/BoM line items must reconcile with the pricing schedule.',
  },
  {
    key: 'joint_venture',
...
export const MANDATORY_LANGUAGE =
  /(must|shall|is\s*(\/\s*are)?\s*required|are\s*required|failing\s*which|no\s*(bid|tender|offer)\s*will\s*be|will\s*(be\s*)?(disqualified|rejected|returned|excluded))/i

/** Disqualifying language — escalates risk to CRITICAL_DISQUALIFIER. */
export const DISQUALIFIER_LANGUAGE =
  /(disqualif|rejected|non[- ]responsive|will\s*not\s*be\s*(considered|evaluated|accepted)|fail(s|ure|ing)?\s*to\s*(comply|submit|provide|furnish)|failing\s*which|returnable)/i
