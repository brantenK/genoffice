...
export const BUNDLED_FONTS = new Set([
  'Noto Sans CJK SC',
  'Noto Serif CJK SC',
  'ExampleOffice Sans KR',
  'ExampleOffice Serif KR',
  'ExampleOffice Gothic KR',
  'ExampleOffice Tamil',
  'ExampleOffice Fullwidth TC',
  'ExampleOffice Songti SC',
  'Carlito GO',
  'Caladea',
  'Liberation Serif',
  'Liberation Sans',
...
export function cssFontFamily(font: string, followAltName = true): string {
  ...
  // 'ExampleOffice Box Drawing' (fonts.css): U+2500 rules would otherwise fall to
  // the fullwidth CJK subset and overflow the column
  const BOX = 'ExampleOffice Box Drawing'
  ...
  // 'ExampleOffice PUA Blank' keeps AI-residue PUA tokens invisible like Word
  // (Calibri/Carlito would otherwise supply a box .notdef for them).
  if (f.includes('aptos')) {
    return `${chain(font, 'Calibri', 'Carlito GO', 'ExampleOffice PUA Blank', CJK_SANS)},sans-serif`
  }
  ...
  // 'ExampleOffice Songti SC' (fonts.css local() alias of Songti SC): macOS Chromium
  ...
  if (f.includes('simsun') || f.includes('宋体') || f.includes('nsimsun')) {
    return `${chain(font, 'ExampleOffice Songti SC', 'STSong', 'SimSun', CJK_SERIF)},serif`
  }
  ...
  // 'ExampleOffice *' entries are CJK-only local() aliases (fonts.css): the underlying
  // system faces draw Cyrillic/Greek fullwidth, so those scripts must pass through
  const JA_SANS = ['Yu Gothic', 'ExampleOffice Hiragino Sans', 'Meiryo', 'Noto Sans JP']
  const JA_SERIF = [
    'Yu Mincho',
    'ExampleOffice Hiragino Mincho',
    'ExampleOffice MS Mincho',
    'Noto Serif JP',
  ]
  const KO_SANS = ['Malgun Gothic', 'ExampleOffice Sans KR', 'Apple SD Gothic Neo', 'Noto Sans KR']
  const KO_SERIF = ['ExampleOffice Batang', 'ExampleOffice Serif KR', 'ExampleOffice Myungjo', 'Noto Serif KR']
  const TC_SANS = ['Microsoft JhengHei', 'PingFang TC', 'ExampleOffice Heiti TC', 'Noto Sans TC']
  // 'ExampleOffice Fullwidth TC' (fonts.css): fullwidth U+FF0D/FF0F/FF3C/FF3F/FF5E whose Songti TC glyphs look half-width
  const TC_SERIF = ['ExampleOffice MingLiU', 'ExampleOffice Fullwidth TC', 'Songti TC', 'Noto Serif TC']
  const SC_SANS = ['PingFang SC', 'Microsoft YaHei', CJK_SANS]
  const SC_SERIF = ['ExampleOffice Songti SC', 'STSong', 'SimSun', CJK_SERIF]
  const nfkc = font.normalize('NFKC')
  ...
  if (
    ...
    if (/nanum ?gothic|나눔 ?고딕/i.test(nfkc)) {
      return `${chain(font, 'ExampleOffice Gothic KR', ...KO_SANS)},sans-serif`
    }
    ...
    if (/(?:batang|gulim|dotum|gungsuh) ?che\b|(?:바탕|굴림|돋움|궁서)체/i.test(nfkc)) {
      ...
      return `${chain(font, 'ExampleOffice Che Latin KR', ...(cheGungsuh ? ['GungSeo'] : []), ...(cheSerif ? KO_SERIF : KO_SANS))},${cheSerif ? 'serif' : 'sans-serif'}`
    }
  ...
  if (/nyala|ebrima|abyssinica|ethiopic/i.test(nfkc)) {
    return `${chain(font, 'ExampleOffice Ethiopic')},sans-serif`
  }
  ...
  if (/tamil|latha|vijaya|inaimathi/i.test(nfkc)) {
    return `${chain(font, 'ExampleOffice Tamil', 'InaiMathi', 'Tamil MN', 'Tamil Sangam MN')},sans-serif`
  }
  ...
  const pua = latinNamed ? ['ExampleOffice PUA Blank'] : []
