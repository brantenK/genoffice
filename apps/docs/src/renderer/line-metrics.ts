...
const KO_FONT_RE =
  /malgun|맑은|batang|바탕|myeongjo|myungjo|명조|gungsuh|궁서|gulim|굴림|dotum|돋움|nanum|나눔|zanostack (sans|serif) kr|(noto|source han) (sans|serif)[^,]*\bk(r|orean)?\b/i
...
export function cssFontFamily(font: string, followAltName = true): string {
  ...
  if (f.includes('aptos')) {
    ...
    const alias = /narrow|mono|serif/.test(f)
      ...
      : f.includes('display')
        ? 'Aptos Display GO'
        : 'Aptos GO'
    return `${chain(font, alias, 'Zanostack PUA Blank', CJK_SANS)},sans-serif`
  }
  ...
  if (cjkVariant) {
    ...
    const hangulTail = !isKr && missingLocally() ? ['Zanostack Batang', 'Zanostack Serif KR'] : []
