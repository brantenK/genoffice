/**
 * Zano Office's own Genspark identity: device-code login (office_addin_auth,
 ...
 * subprocesses) to Zano Office. Stored in ~/.genoffice/auth.json, deliberately
...
/** The Zano Office-named api key; '' when not signed in. Cached (invalidated by login/logout). */
...
/**
 * Starts the Zano Office device-code login, cancelling a previous in-flight one
...
/**
 * Signs out of Zano Office only: best-effort server-side revoke of the
