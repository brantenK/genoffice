/**
 * Zanostack's own Genspark identity: device-code login (office_addin_auth,
 ...
 * subprocesses) to Zanostack. Stored in ~/.genoffice/auth.json, deliberately
...
/** The Zanostack-named api key; '' when not signed in. Cached (invalidated by login/logout). */
...
/**
 * Starts the Zanostack device-code login, cancelling a previous in-flight one
...
/**
 * Signs out of Zanostack only: best-effort server-side revoke of the
