...
You are reviewer_1_books_m5_fresh.
Your working directory is:
c:\Users\brant\OneDrive\Documents\GenOffice\genoffice\.agents\reviewer_1_books_m5_fresh
...
2. Feature F18 (Invariants & Ledger Posting Tests):
   - Check `invariants.test.ts`: Does it test strict double-entry equality ($\sum \text{Debits} = \sum \text{Credits}$), `round2` floating-point edge cases, `calculateInvoiceTotals` (15%/0% VAT and negative discounts), and party balance invariants?
   - Check `ledger-posting.test.ts`: Does it test Sales Invoices and Purchase Bills journal posting, Draft-to-Draft transitions, payment settlements, and invoice deletion reversals?
...
Deliver your verdict (APPROVE or REQUEST_CHANGES) with supporting evidence in `c:\Users\brant\OneDrive\Documents\GenOffice\genoffice\.agents\reviewer_1_books_m5_fresh\handoff.md` and notify the orchestrator via send_message.
