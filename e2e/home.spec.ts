...
test.describe('home screen', () => {
  test('shows hero, quick-create cards and tab bar', async () => {
    ...
    try {
      ...
      // nine app quick-create cards plus the "Open file" browse card
      await expect(page.locator('.quick-card')).toHaveCount(10)
      ...
      await expect(page.locator('.quick-card').nth(5)).toContainText('CRM')
      await expect(page.locator('.quick-card').nth(6)).toContainText('Tenders')
      await expect(page.locator('.quick-card').nth(7)).toContainText('Books')
      await expect(page.locator('.quick-card').nth(8)).toContainText('AI PDF')
