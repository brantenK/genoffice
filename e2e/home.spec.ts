...
test.describe('home screen', () => {
  test('shows hero, quick-create cards and tab bar', async () => {
    ...
    try {
      ...
      // eight app quick-create cards (Docs, Sheets, Slides, Markdown, CRM, Tenders, Books, PDF) plus "Open file"
      await expect(page.locator('.quick-card')).toHaveCount(9)
      ...
      await expect(page.locator('.quick-card').nth(4)).toContainText('CRM')
      await expect(page.locator('.quick-card').nth(5)).toContainText('Tenders')
      await expect(page.locator('.quick-card').nth(6)).toContainText('Books')
      await expect(page.locator('.quick-card').nth(7)).toContainText('AI PDF')
