...
- **`e2e/` is now in sync with the fork's Home** — 145+ passed / 2 failed / 7 skipped, against 116 / 32 / 7 before the port. The fork's Home redesign had left upstream's specs driving a card row that no longer exists; the specs were ported to the fork's markup rather than the reverse, because the user's UI is the settled thing.
...
- **Two genuine product defects, found by those specs and deliberately left unpatched** (UI decisions are settled; both are upstream code):
  - **The tenders drawer close control is unreachable by pointer.** The workspace toolbar is `position: sticky; z-index: 30` (`apps/tenders/src/renderer/src/styles/responsive.css:71-75`) while the shared `Drawer` aside is `z-20` (`apps/tenders/src/renderer/src/components/Drawer.tsx:59`); the drawer is also `inset-y-0`, so its title row — title and X — paints _underneath_ the toolbar. Measured with the drawer open: the X occupies `{x:1306,y:14,w:37,h:29}` and `document.elementFromPoint(1325,29)` returns the toolbar's overflow trigger. Every `Drawer`-based aside is affected (readiness, vault, milestones); `TrashDrawer` uses `z-30` and is immune. Users can only close these with Escape.
